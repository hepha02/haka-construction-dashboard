import{createClient as j}from"https://esm.sh/@supabase/supabase-js@2.49.1";(function(){const t=document.createElement("link").relList;if(t&&t.supports&&t.supports("modulepreload"))return;for(const a of document.querySelectorAll('link[rel="modulepreload"]'))r(a);new MutationObserver(a=>{for(const s of a)if(s.type==="childList")for(const o of s.addedNodes)o.tagName==="LINK"&&o.rel==="modulepreload"&&r(o)}).observe(document,{childList:!0,subtree:!0});function n(a){const s={};return a.integrity&&(s.integrity=a.integrity),a.referrerPolicy&&(s.referrerPolicy=a.referrerPolicy),a.crossOrigin==="use-credentials"?s.credentials="include":a.crossOrigin==="anonymous"?s.credentials="omit":s.credentials="same-origin",s}function r(a){if(a.ep)return;a.ep=!0;const s=n(a);fetch(a.href,s)}})();const T="https://yqemtsbdnypgmkuyncxh.supabase.co",P="eyJhbGciOiJIUzI1NiIsInR5cCI6IkpXVCJ9.eyJpc3MiOiJzdXBhYmFzZSIsInJlZiI6InlxZW10c2JkbnlwZ21rdXluY3hoIiwicm9sZSI6ImFub24iLCJpYXQiOjE3ODAyNjYwMTUsImV4cCI6MjA5NTg0MjAxNX0.gwgdCncqRKKgC8ebj7qIdT-vA4J-wOVd2O9DSa7xEOs",i=j(T,P),N="construction-start-files",d={payments:[],stores:[{id:1,name:"성수 플래그십",area:52,status:"완료",budget:21e7,spent:1984e5},{id:2,name:"부산 센텀",area:47,status:"진행중",budget:186e6,spent:1227e5},{id:3,name:"대전 둔산",area:39,status:"진행중",budget:144e6,spent:882e5},{id:4,name:"제주 노형",area:42,status:"미착공",budget:158e6,spent:0}],vendors:[{id:1,name:"도원인테리어",category:"시공",bank:"신한은행",account_number:"110-000-000001",account_holder:"도원인테리어",risk:"정상",total:1245e5},{id:2,name:"한빛전기",category:"전기",bank:"국민은행",account_number:"004-000-000002",account_holder:"한빛전기",risk:"정상",total:738e5},{id:3,name:"서진설비",category:"설비",bank:"하나은행",account_number:"352-000-000003",account_holder:"서진설비",risk:"증빙확인",total:412e5}],userRoles:[],paymentItems:[],constructionStarts:[]},R=["대시보드","엑셀 업로드","공사 시작 접수","결제 신청","업체/계좌 관리","매장별 공사 관리","진열장 원가 배분","견적서 생성","계약서 생성","은행 이체 파일 생성","관리자 설정"],x={"전체 관리자":R,"인테리어 공사실장":["공사 시작 접수","결제 신청","업체/계좌 관리","진열장 원가 배분"]},E={admin:"전체 관리자",interior_manager:"인테리어 공사실장","전체 관리자":"전체 관리자","인테리어 공사실장":"인테리어 공사실장"},V={"엑셀 업로드":["결제 신청 내역 엑셀 업로드","필수 컬럼 검증","중복/오류 행 표시"],"진열장 원가 배분":["매장별 진열장 비용 배분","공용 비용 자동 분배","평당 원가 반영"],"견적서 생성":["매장/업체 기준 견적서 생성","공사항목별 금액 자동 합산","PDF/문서 다운로드"],"계약서 생성":["업체 정보 기반 계약서 생성","계좌/사업자 정보 자동 반영","계약 상태 관리"],"은행 이체 파일 생성":["승인된 결제 건만 추출","은행 업로드용 파일 생성","이체 전 검증"],"관리자 설정":["사용자 권한","승인 단계","상태/분류 코드 관리"]};let p=d,l="대시보드",w="인테리어 공사실장",f=null;const v=e=>new Intl.NumberFormat("ko-KR",{style:"currency",currency:"KRW",maximumFractionDigits:0}).format(e||0),M=()=>new Date().toISOString().slice(0,10),C=e=>Number(String(e).replace(/[^\d]/g,"")),m=e=>String(e||"").replace(/&/g,"&amp;").replace(/"/g,"&quot;").replace(/</g,"&lt;").replace(/>/g,"&gt;"),K=e=>String(e||"file").replace(/[^\w.\-가-힣]/g,"_"),q=e=>({승인:"green",신청:"amber",반려:"red",진행중:"blue",완료:"green",미착공:"gray",정상:"green",증빙확인:"amber"})[e]||"gray";async function S(){if(!i)return d;const[e,t,n,r,a,s]=await Promise.all([i.from("payments").select("*").order("requested_at",{ascending:!1}).order("id",{ascending:!1}).limit(12),i.from("stores").select("*").order("id",{ascending:!0}),i.from("vendors").select("*").order("id",{ascending:!0}),i.from("construction_starts").select("*").order("created_at",{ascending:!1}).order("id",{ascending:!1}).limit(30),i.from("user_roles").select("email, role, created_at").order("email",{ascending:!0}),i.from("construction_cost_parts").select("part_name").order("part_name",{ascending:!0})]),o=s.error?d.paymentItems:[...new Set(s.data.map(c=>c.part_name).filter(Boolean))];return{payments:e.error?d.payments:e.data,stores:t.error?d.stores:t.data,vendors:n.error?d.vendors:n.data,constructionStarts:r.error?d.constructionStarts:r.data,userRoles:a.error?d.userRoles:a.data,paymentItems:o}}async function L(e){if(!i||!(e!=null&&e.email))return"인테리어 공사실장";const{data:t,error:n}=await i.from("user_roles").select("role").ilike("email",e.email).maybeSingle();return n||!(t!=null&&t.role)?"인테리어 공사실장":E[t.role]||"인테리어 공사실장"}async function U(){var t;if(!i){p=await S(),w="전체 관리자",g();return}const{data:e}=await i.auth.getSession();if(f=((t=e.session)==null?void 0:t.user)||null,!f){h();return}w=await L(f),l=b()[0],p=await S(),g()}function B(e,t,n){return e.payments.find(r=>{const a=r.vendor.trim()===t.trim(),s=Math.abs(r.amount-n)/Math.max(n,1);return a&&s<=.1})}async function D(e,t){const n=Array.from(e||[]).filter(a=>a.size>0);if(!n.length)return[];if(!i)return n.map(a=>({name:a.name,type:a.type,size:a.size,path:"",url:""}));const r=[];for(const a of n){const s=`${(f==null?void 0:f.id)||"user"}/${t}/${Date.now()}-${crypto.randomUUID()}-${K(a.name)}`,{error:o}=await i.storage.from(N).upload(s,a,{contentType:a.type,upsert:!1});if(o)throw o;const{data:c}=i.storage.from(N).getPublicUrl(s);r.push({name:a.name,type:a.type,size:a.size,path:s,url:c.publicUrl})}return r}async function J(e){e.preventDefault();const t=e.currentTarget,n=t.querySelector("button[type='submit']"),r=t.querySelector("[data-form-message]"),a=new FormData(t),s=String(a.get("store")||"").trim(),o=String(a.get("vendor")||"").trim(),c=String(a.get("payment_item")||"").trim(),k=C(a.get("amount")),_=B(p,o,k);if(!s||!o||!c||!k){r.textContent="매장명, 업체, 결제 항목, 신청 금액을 모두 입력해 주세요.",r.className="form-message error";return}n.disabled=!0,n.textContent="저장 중",r.textContent=_?`중복 의심: ${_.store} / ${v(_.amount)} 건과 비슷합니다.`:"신청 건을 저장하고 있습니다.",r.className=_?"form-message warning":"form-message";const u={store:s,vendor:o,payment_item:c,amount:k,status:"신청",requested_at:M()};if(!i)d.payments=[{id:Date.now(),...u},...d.payments];else{const{error:$}=await i.from("payments").insert(u);if($){n.disabled=!1,n.textContent="검토 요청 생성",r.textContent=`저장 실패: ${$.message}`,r.className="form-message error";return}}t.reset(),p=await S(),g(_?"신청이 저장됐습니다. 중복 의심 건은 결제 검토에서 확인하세요.":"신청이 저장됐습니다.")}async function z(e){e.preventDefault();const t=e.currentTarget,n=t.querySelector("button[type='submit']"),r=t.querySelector("[data-vendor-message]"),a=new FormData(t),s={name:String(a.get("name")||"").trim(),category:String(a.get("category")||"").trim(),bank:String(a.get("bank")||"").trim(),account_number:String(a.get("account_number")||"").trim(),account_holder:String(a.get("account_holder")||"").trim(),risk:"정상",total:0};if(!s.name||!s.category||!s.bank||!s.account_number||!s.account_holder){r.textContent="업체명, 분류, 은행, 계좌번호, 예금주를 모두 입력해 주세요.",r.className="form-message error";return}if(n.disabled=!0,n.textContent="저장 중",r.textContent="업체/계좌 정보를 저장하고 있습니다.",r.className="form-message",!i)d.vendors=[{id:Date.now(),...s},...d.vendors];else{const{error:o}=await i.from("vendors").insert(s);if(o){n.disabled=!1,n.textContent="업체 저장",r.textContent=`저장 실패: ${o.message}`,r.className="form-message error";return}}t.reset(),p=await S(),l="업체/계좌 관리",g("업체/계좌 정보가 저장됐습니다.")}async function H(e){e.preventDefault();const t=e.currentTarget,n=t.querySelector("button[type='submit']"),r=t.querySelector("[data-store-message]"),a=new FormData(t),s={name:String(a.get("name")||"").trim(),area:Number(a.get("area")),status:String(a.get("status")||"미착공"),budget:C(a.get("budget")),spent:C(a.get("spent"))};if(!s.name||!s.area||!s.budget){r.textContent="매장명, 면적, 예산을 입력해 주세요.",r.className="form-message error";return}if(n.disabled=!0,n.textContent="저장 중",r.textContent="매장 공사 정보를 저장하고 있습니다.",r.className="form-message",!i)d.stores=[{id:Date.now(),...s},...d.stores];else{const{error:o}=await i.from("stores").insert(s);if(o){n.disabled=!1,n.textContent="매장 저장",r.textContent=`저장 실패: ${o.message}`,r.className="form-message error";return}}t.reset(),p=await S(),l="매장별 공사 관리",g("매장 공사 정보가 저장됐습니다.")}async function X(e){e.preventDefault();const t=e.currentTarget,n=t.querySelector("button[type='submit']"),r=t.querySelector("[data-construction-start-message]"),a=new FormData(t),s={store_name:String(a.get("store_name")||"").trim(),area:Number(a.get("area")),fixture_count:Number(a.get("fixture_count")||0),table_count:Number(a.get("table_count")||0),sign_count:Number(a.get("sign_count")||0),special_notes:String(a.get("special_notes")||"").trim()};if(!s.store_name||!s.area){r.textContent="매장명과 평수는 꼭 입력해 주세요.",r.className="form-message error";return}n.disabled=!0,n.textContent="저장 중",r.textContent="도면과 사진 파일을 업로드하고 있습니다.",r.className="form-message";try{s.drawing_files=await D(a.getAll("drawing_files"),"drawings"),s.base_photo_files=await D(a.getAll("base_photo_files"),"base-photos")}catch(o){n.disabled=!1,n.textContent="공사 시작 정보 저장",r.textContent=`파일 업로드 실패: ${o.message}`,r.className="form-message error";return}if(r.textContent="공사 시작 정보를 저장하고 있습니다.",!i)d.constructionStarts=[{id:Date.now(),created_at:new Date().toISOString(),...s},...d.constructionStarts];else{const{error:o}=await i.from("construction_starts").insert(s);if(o){n.disabled=!1,n.textContent="공사 시작 정보 저장",r.textContent=`저장 실패: ${o.message}`,r.className="form-message error";return}}t.reset(),p=await S(),l="공사 시작 접수",g("공사 시작 정보가 저장됐습니다. 결제 신청 매장 검색에도 반영됐습니다.")}async function Y(e,t){if(!(!e||!["승인","반려"].includes(t))){if(!i)d.payments=d.payments.map(n=>n.id===e?{...n,status:t}:n);else{const{error:n}=await i.from("payments").update({status:t}).eq("id",e).eq("status","신청");if(n){g(`상태 변경 실패: ${n.message}`);return}}p=await S(),l="결제 신청",g(`결제 신청이 ${t} 처리됐습니다.`)}}function Z(e){const t=e.stores.filter(u=>u.status==="완료").length,n=e.stores.filter(u=>u.status==="진행중").length,r=e.stores.filter(u=>u.document_required).length,a=e.payments.filter(u=>u.status==="승인").reduce((u,$)=>u+$.amount,0),s=e.stores.reduce((u,$)=>u+Number($.spent||0),0),o=e.payments.filter(u=>u.status==="신청").length,c=e.stores.filter(u=>String(u.name||"").includes("직영점")).length,k=e.stores.reduce((u,$)=>u+Number($.area||0),0),_=Math.round(s/Math.max(k,1));return[["완료된 매장",`${t}개`,"엑셀 공사 상태 기준"],["진행중인 매장",`${n}개`,"시공 또는 비용 검수 중"],["전체 공사비",v(s),"엑셀 합계 기준"],["문서 생성 대상",`${r}개`,"강남압구정 행부터 아래"],["대기중인 결제",`${o}건`,"승인 전 검토 필요"],["직영점",`${c}개`,"지점명 기준"],["승인된 결제",v(a),"지급 승인 완료"],["평균 평당 원가",v(_),"엑셀 합계/평수 기준"]]}function y(e,t){return`
  ...
  `}function W(e){return e.payments.map(t=>`
      ...
      </tr>`)}function I(e,t=""){const n=Array.isArray(e)?e:[];return n.length?n.map(r=>r.url?`<a href="${m(r.url)}" target="_blank" rel="noreferrer">${m(r.name||"파일")}</a>`:`<span>${m(r.name||"파일")}</span>`).join("<br />"):t||"-"}function Q(e){return e.constructionStarts.map(t=>`
      <tr>
        <td>${t.store_name}</td>
        <td>${t.area}평</td>
        <td>${t.fixture_count||0}</td>
      ...
      </tr>`)}function tt(e){return e.vendors.map(t=>`<option value="${m(t.name)}">${m(t.name)} / ${m(t.bank)} ${m(t.account_number||"")}</option>`).join("")}function et(e){const n=[...e.constructionStarts.map(a=>({name:a.store_name,area:a.area,status:"공사 시작 접수"})),...e.stores],r=new Set;return n.filter(a=>{const s=String(a.name||"").trim();return!s||r.has(s)?!1:(r.add(s),!0)}).map(a=>`<option value="${m(a.name)}">${m(a.name)} / ${m(a.area)}평 / ${m(a.status)}</option>`).join("")}function at(e){return e.paymentItems.map(t=>`<option value="${m(t)}">${m(t)}</option>`).join("")}function O(){return`
  ...
  `}function ot(e){return`
    ...
    <section class="grid two">
      ...
      <article class="panel">
        ...
        ${y(["지역","매장","진열장","평수","공사비 합계","상태","문서"],A(e))}
      </article>
    </section>
    ...
    <section class="grid lower">
      <article class="panel">
        ...
        ${y(["업체","분류","은행","계좌번호","예금주","누적 지급","상태"],F(e))}
      </article>
      ${O()}
    </section>
  ...
  `}function ct(e){return`
    <section class="grid two">
      ...
      <article class="panel">
        ...
        ${y(["매장","평수","진열장","테이블","광고판","도면","기초 사진","특이사항"],Q(e))}
      </article>
    </section>
  ...
  `}function dt(e){return e.userRoles.map(t=>`
      <tr>
        <td>${t.email}</td>
        <td><span class="badge ${t.role==="admin"?"green":"blue"}">${E[t.role]||t.role}</span></td>
        <td>${t.created_at?String(t.created_at).slice(0,10):"-"}</td>
      </tr>`)}function mt(){return Object.entries(x).map(([e,t])=>`
      <tr>
        <td><strong>${e}</strong></td>
        <td>${t.map(n=>`<span class="menu-chip">${n}</span>`).join("")}</td>
        <td>${t.length}개</td>
      </tr>`)}function pt(e){return`
    <section class="grid two">
      <article class="panel">
        <div class="panel-head">
          <h2>사용자 권한</h2>
      ...
      <article class="panel">
        ...
        ${y(["권한","볼 수 있는 메뉴","메뉴 수"],mt())}
      </article>
    </section>
  `}function ft(e){const t=V[e]||["기능 범위 정의","입력 항목 확정","데이터 연결"];return`
  ...
  `}function gt(e){return l==="대시보드"?ot(e):l==="공사 시작 접수"?ct(e):l==="결제 신청"?it(e):l==="업체/계좌 관리"?lt(e):l==="매장별 공사 관리"?ut(e):l==="관리자 설정"?pt(e):ft(l)}function b(){return x[w]||R}function bt(){return`
  ...
  `,document.querySelector("#auth-form").addEventListener("submit",ht),document.querySelector("[data-auth-action='signup']").addEventListener("click",yt),document.querySelector("[data-auth-action='resend']").addEventListener("click",$t)}async function ht(e){e.preventDefault();const t=new FormData(e.currentTarget),n=String(t.get("email")||"").trim(),r=String(t.get("password")||"");if(!n||!r){h("이메일과 비밀번호를 입력해 주세요.");return}const{data:a,error:s}=await i.auth.signInWithPassword({email:n,password:r});if(s){h(`로그인 실패: ${s.message}`);return}f=a.user,w=await L(f),l=b()[0],p=await S(),g()}async function yt(){const e=document.querySelector("#auth-form"),t=new FormData(e),n=String(t.get("email")||"").trim(),r=String(t.get("password")||"");if(!n||!r){h("계정을 만들 이메일과 비밀번호를 입력해 주세요.");return}const{error:a}=await i.auth.signUp({email:n,password:r,options:{emailRedirectTo:window.location.origin+window.location.pathname}});if(a){h(`회원가입 실패: ${a.message}`);return}h("계정이 생성됐습니다. 이메일 확인이 필요한 경우 메일 인증 후 로그인해 주세요.")}async function $t(){const e=document.querySelector("#auth-form"),t=new FormData(e),n=String(t.get("email")||"").trim();if(!n){h("인증메일을 다시 받을 이메일을 입력해 주세요.");return}const{error:r}=await i.auth.resend({type:"signup",email:n,options:{emailRedirectTo:window.location.origin+window.location.pathname}});if(r){h(`인증메일 재발송 실패: ${r.message}`);return}h("인증메일을 다시 보냈습니다. 새로 받은 메일의 링크를 눌러 주세요.")}async function vt(){await i.auth.signOut(),f=null,w="인테리어 공사실장",h("로그아웃되었습니다.")}function g(e=""){var o;const t=document.querySelector("#app");t.className="",b().includes(l)||(l=b()[0]),t.innerHTML=`
    <aside class="sidebar">
      <div class="brand">
        <span class="brand-mark">H</span>
        <div>
  ...
  `,document.querySelectorAll("[data-view]").forEach(c=>{c.addEventListener("click",()=>{l=c.dataset.view,g()})}),document.querySelectorAll("[data-role]").forEach(c=>{c.addEventListener("click",()=>{w=c.dataset.role,b().includes(l)||(l=b()[0]),g()})}),(o=document.querySelector("[data-sign-out]"))==null||o.addEventListener("click",vt),document.querySelectorAll("[data-view-link]").forEach(c=>{c.addEventListener("click",()=>{b().includes(c.dataset.viewLink)&&(l=c.dataset.viewLink),g()})});const n=document.querySelector("#payment-form");n&&n.addEventListener("submit",J);const r=document.querySelector("#vendor-form");r&&r.addEventListener("submit",z);const a=document.querySelector("#store-form");a&&a.addEventListener("submit",H);const s=document.querySelector("#construction-start-form");s&&s.addEventListener("submit",X),document.querySelectorAll("[data-payment-id][data-payment-status]").forEach(c=>{c.addEventListener("click",()=>{Y(Number(c.dataset.paymentId),c.dataset.paymentStatus)})})}U();
