import{createClient as E}from"https://esm.sh/@supabase/supabase-js@2.49.1";(function(){const t=document.createElement("link").relList;if(t&&t.supports&&t.supports("modulepreload"))return;for(const n of document.querySelectorAll('link[rel="modulepreload"]'))s(n);new MutationObserver(n=>{for(const r of n)if(r.type==="childList")for(const o of r.addedNodes)o.tagName==="LINK"&&o.rel==="modulepreload"&&s(o)}).observe(document,{childList:!0,subtree:!0});function a(n){const r={};return n.integrity&&(r.integrity=n.integrity),n.referrerPolicy&&(r.referrerPolicy=n.referrerPolicy),n.crossOrigin==="use-credentials"?r.credentials="include":n.crossOrigin==="anonymous"?r.credentials="omit":r.credentials="same-origin",r}function s(n){if(n.ep)return;n.ep=!0;const r=a(n);fetch(n.href,r)}})();const A="https://yqemtsbdnypgmkuyncxh.supabase.co",I="eyJhbGciOiJIUzI1NiIsInR5cCI6IkpXVCJ9.eyJpc3MiOiJzdXBhYmFzZSIsInJlZiI6InlxZW10c2JkbnlwZ21rdXluY3hoIiwicm9sZSI6ImFub24iLCJpYXQiOjE3ODAyNjYwMTUsImV4cCI6MjA5NTg0MjAxNX0.gwgdCncqRKKgC8ebj7qIdT-vA4J-wOVd2O9DSa7xEOs",c=E(A,I),u={payments:[{id:1,store:"성수 플래그십",vendor:"한빛전기",amount:184e5,status:"승인",requested_at:"2026-05-28"},{id:2,store:"부산 센텀",vendor:"도원인테리어",amount:327e5,status:"신청",requested_at:"2026-05-27"},{id:3,store:"대전 둔산",vendor:"서진설비",amount:98e5,status:"반려",requested_at:"2026-05-26"},{id:4,store:"제주 노형",vendor:"제이건축",amount:241e5,status:"승인",requested_at:"2026-05-25"}],stores:[{id:1,name:"성수 플래그십",area:52,status:"완료",budget:21e7,spent:1984e5},{id:2,name:"부산 센텀",area:47,status:"진행중",budget:186e6,spent:1227e5},{id:3,name:"대전 둔산",area:39,status:"진행중",budget:144e6,spent:882e5},{id:4,name:"제주 노형",area:42,status:"미착공",budget:158e6,spent:0}],vendors:[{id:1,name:"도원인테리어",category:"시공",bank:"신한은행",account_number:"110-000-000001",account_holder:"도원인테리어",risk:"정상",total:1245e5},{id:2,name:"한빛전기",category:"전기",bank:"국민은행",account_number:"004-000-000002",account_holder:"한빛전기",risk:"정상",total:738e5},{id:3,name:"서진설비",category:"설비",bank:"하나은행",account_number:"352-000-000003",account_holder:"서진설비",risk:"증빙확인",total:412e5}]},D=["대시보드","엑셀 업로드","결제 신청","업체/계좌 관리","매장별 공사 관리","진열장 원가 배분","견적서 생성","계약서 생성","은행 이체 파일 생성","관리자 설정"],O={"전체 관리자":D,"인테리어 공사실장":["결제 신청","진열장 원가 배분"]},F={admin:"전체 관리자",interior_manager:"인테리어 공사실장","전체 관리자":"전체 관리자","인테리어 공사실장":"인테리어 공사실장"},R={"엑셀 업로드":["결제 신청 내역 엑셀 업로드","필수 컬럼 검증","중복/오류 행 표시"],"진열장 원가 배분":["매장별 진열장 비용 배분","공용 비용 자동 분배","평당 원가 반영"],"견적서 생성":["매장/업체 기준 견적서 생성","공사항목별 금액 자동 합산","PDF/문서 다운로드"],"계약서 생성":["업체 정보 기반 계약서 생성","계좌/사업자 정보 자동 반영","계약 상태 관리"],"은행 이체 파일 생성":["승인된 결제 건만 추출","은행 업로드용 파일 생성","이체 전 검증"],"관리자 설정":["사용자 권한","승인 단계","상태/분류 코드 관리"]};let m=u,l="대시보드",h="인테리어 공사실장",b=null;const g=e=>new Intl.NumberFormat("ko-KR",{style:"currency",currency:"KRW",maximumFractionDigits:0}).format(e||0),j=()=>new Date().toISOString().slice(0,10),_=e=>Number(String(e).replace(/[^\d]/g,"")),q=e=>({승인:"green",신청:"amber",반려:"red",진행중:"blue",완료:"green",미착공:"gray",정상:"green",증빙확인:"amber"})[e]||"gray";async function y(){if(!c)return u;const[e,t,a]=await Promise.all([c.from("payments").select("*").order("requested_at",{ascending:!1}).order("id",{ascending:!1}).limit(12),c.from("stores").select("*").order("id",{ascending:!0}),c.from("vendors").select("*").order("id",{ascending:!0})]);return{payments:e.error?u.payments:e.data,stores:t.error?u.stores:t.data,vendors:a.error?u.vendors:a.data}}async function C(e){if(!c||!(e!=null&&e.email))return"인테리어 공사실장";const{data:t,error:a}=await c.from("user_roles").select("role").ilike("email",e.email).maybeSingle();return a||!(t!=null&&t.role)?"인테리어 공사실장":F[t.role]||"인테리어 공사실장"}async function M(){var t;if(!c){m=await y(),h="전체 관리자",d();return}const{data:e}=await c.auth.getSession();if(b=((t=e.session)==null?void 0:t.user)||null,!b){f();return}h=await C(b),l=p()[0],m=await y(),d()}function P(e,t,a){return e.payments.find(s=>{const n=s.vendor.trim()===t.trim(),r=Math.abs(s.amount-a)/Math.max(a,1);return n&&r<=.1})}async function T(e){e.preventDefault();const t=e.currentTarget,a=t.querySelector("button[type='submit']"),s=t.querySelector("[data-form-message]"),n=new FormData(t),r=String(n.get("store")||"").trim(),o=String(n.get("vendor")||"").trim(),w=_(n.get("amount")),v=P(m,o,w);if(!r||!o||!w){s.textContent="매장명, 업체, 신청 금액을 모두 입력해 주세요.",s.className="form-message error";return}a.disabled=!0,a.textContent="저장 중",s.textContent=v?`중복 의심: ${v.store} / ${g(v.amount)} 건과 비슷합니다.`:"신청 건을 저장하고 있습니다.",s.className=v?"form-message warning":"form-message";const k={store:r,vendor:o,amount:w,status:"신청",requested_at:j()};if(!c)u.payments=[{id:Date.now(),...k},...u.payments];else{const{error:i}=await c.from("payments").insert(k);if(i){a.disabled=!1,a.textContent="검토 요청 생성",s.textContent=`저장 실패: ${i.message}`,s.className="form-message error";return}}t.reset(),m=await y(),d(v?"신청이 저장됐습니다. 중복 의심 건은 결제 검토에서 확인하세요.":"신청이 저장됐습니다.")}async function V(e){e.preventDefault();const t=e.currentTarget,a=t.querySelector("button[type='submit']"),s=t.querySelector("[data-vendor-message]"),n=new FormData(t),r={name:String(n.get("name")||"").trim(),category:String(n.get("category")||"").trim(),bank:String(n.get("bank")||"").trim(),account_number:String(n.get("account_number")||"").trim(),account_holder:String(n.get("account_holder")||"").trim(),risk:"정상",total:0};if(!r.name||!r.category||!r.bank||!r.account_number||!r.account_holder){s.textContent="업체명, 분류, 은행, 계좌번호, 예금주를 모두 입력해 주세요.",s.className="form-message error";return}if(a.disabled=!0,a.textContent="저장 중",s.textContent="업체/계좌 정보를 저장하고 있습니다.",s.className="form-message",!c)u.vendors=[{id:Date.now(),...r},...u.vendors];else{const{error:o}=await c.from("vendors").insert(r);if(o){a.disabled=!1,a.textContent="업체 저장",s.textContent=`저장 실패: ${o.message}`,s.className="form-message error";return}}t.reset(),m=await y(),l="업체/계좌 관리",d("업체/계좌 정보가 저장됐습니다.")}async function J(e){e.preventDefault();const t=e.currentTarget,a=t.querySelector("button[type='submit']"),s=t.querySelector("[data-store-message]"),n=new FormData(t),r={name:String(n.get("name")||"").trim(),area:Number(n.get("area")),status:String(n.get("status")||"미착공"),budget:_(n.get("budget")),spent:_(n.get("spent"))};if(!r.name||!r.area||!r.budget){s.textContent="매장명, 면적, 예산을 입력해 주세요.",s.className="form-message error";return}if(a.disabled=!0,a.textContent="저장 중",s.textContent="매장 공사 정보를 저장하고 있습니다.",s.className="form-message",!c)u.stores=[{id:Date.now(),...r},...u.stores];else{const{error:o}=await c.from("stores").insert(r);if(o){a.disabled=!1,a.textContent="매장 저장",s.textContent=`저장 실패: ${o.message}`,s.className="form-message error";return}}t.reset(),m=await y(),l="매장별 공사 관리",d("매장 공사 정보가 저장됐습니다.")}async function K(e,t){if(!(!e||!["승인","반려"].includes(t))){if(!c)u.payments=u.payments.map(a=>a.id===e?{...a,status:t}:a);else{const{error:a}=await c.from("payments").update({status:t}).eq("id",e).eq("status","신청");if(a){d(`상태 변경 실패: ${a.message}`);return}}m=await y(),l="결제 신청",d(`결제 신청이 ${t} 처리됐습니다.`)}}function B(e){const t=e.stores.filter(i=>i.status==="완료").length,a=e.stores.filter(i=>i.status==="진행중").length,s=e.stores.filter(i=>i.document_required).length,n=e.payments.filter(i=>i.status==="승인").reduce((i,S)=>i+S.amount,0),r=e.stores.reduce((i,S)=>i+Number(S.spent||0),0),o=e.payments.filter(i=>i.status==="신청").length,w=e.stores.filter(i=>String(i.name||"").includes("직영점")).length,v=e.stores.reduce((i,S)=>i+Number(S.area||0),0),k=Math.round(r/Math.max(v,1));return[["완료된 매장",`${t}개`,"엑셀 공사 상태 기준"],["진행중인 매장",`${a}개`,"시공 또는 비용 검수 중"],["전체 공사비",g(r),"엑셀 합계 기준"],["문서 생성 대상",`${s}개`,"강남압구정 행부터 아래"],["대기중인 결제",`${o}건`,"승인 전 검토 필요"],["직영점",`${w}개`,"지점명 기준"],["승인된 결제",g(n),"지급 승인 완료"],["평균 평당 원가",g(k),"엑셀 합계/평수 기준"]]}function $(e,t){return`
  ...
  `}function H(e){return e.payments.map(t=>`
      <tr>
        <td>${t.store}</td>
        <td>${t.vendor}</td>
      ...
      </tr>`)}function U(e){return e.payments.map(t=>`
      <tr>
        <td>${t.store}</td>
        <td>${t.vendor}</td>
        <td class="money">${g(t.amount)}</td>
      ...
      </tr>`)}function N(e){return e.stores.map(t=>`
      <tr>
        <td>${t.region||"-"}</td>
        <td>${t.name}</td>
      ...
      </tr>`)}function L(e){return e.vendors.map(t=>`
      <tr>
        <td>${t.name}</td>
        <td>${t.category}</td>
        <td>${t.bank}</td>
        ...
        <td class="money">${g(t.total)}</td>
        <td><span class="badge ${q(t.risk)}">${t.risk}</span></td>
      </tr>`)}function X(e){return e.vendors.map(t=>`<option value="${t.name}">${t.name} / ${t.bank} ${t.account_number||""}</option>`).join("")}function Y(e){return e.stores.map(t=>`<option value="${t.name}">${t.name} / ${t.area}평 / ${t.status}</option>`).join("")}function x(){return`
  ...
  `}function z(e){return`
    ...
    <section class="grid lower">
      <article class="panel">
        ...
        ${$(["업체","분류","은행","계좌번호","예금주","누적 지급","상태"],L(e))}
      </article>
      ${x()}
    </section>
  `}function G(e){return`
  ...
  `}function et(e){const t=R[e]||["기능 범위 정의","입력 항목 확정","데이터 연결"];return`
  ...
  `}function at(e){return l==="대시보드"?z(e):l==="결제 신청"?G(e):l==="업체/계좌 관리"?Q(e):l==="매장별 공사 관리"?tt(e):et(l)}function p(){return O[h]||D}function nt(){return`
  ...
  `,document.querySelector("#auth-form").addEventListener("submit",st),document.querySelector("[data-auth-action='signup']").addEventListener("click",rt),document.querySelector("[data-auth-action='resend']").addEventListener("click",ot)}async function st(e){e.preventDefault();const t=new FormData(e.currentTarget),a=String(t.get("email")||"").trim(),s=String(t.get("password")||"");if(!a||!s){f("이메일과 비밀번호를 입력해 주세요.");return}const{data:n,error:r}=await c.auth.signInWithPassword({email:a,password:s});if(r){f(`로그인 실패: ${r.message}`);return}b=n.user,h=await C(b),l=p()[0],m=await y(),d()}async function rt(){const e=document.querySelector("#auth-form"),t=new FormData(e),a=String(t.get("email")||"").trim(),s=String(t.get("password")||"");if(!a||!s){f("계정을 만들 이메일과 비밀번호를 입력해 주세요.");return}const{error:n}=await c.auth.signUp({email:a,password:s,options:{emailRedirectTo:window.location.origin+window.location.pathname}});if(n){f(`회원가입 실패: ${n.message}`);return}f("계정이 생성됐습니다. 이메일 확인이 필요한 경우 메일 인증 후 로그인해 주세요.")}async function ot(){const e=document.querySelector("#auth-form"),t=new FormData(e),a=String(t.get("email")||"").trim();if(!a){f("인증메일을 다시 받을 이메일을 입력해 주세요.");return}const{error:s}=await c.auth.resend({type:"signup",email:a,options:{emailRedirectTo:window.location.origin+window.location.pathname}});if(s){f(`인증메일 재발송 실패: ${s.message}`);return}f("인증메일을 다시 보냈습니다. 새로 받은 메일의 링크를 눌러 주세요.")}async function it(){await c.auth.signOut(),b=null,h="인테리어 공사실장",f("로그아웃되었습니다.")}function d(e=""){var r;const t=document.querySelector("#app");t.className="",p().includes(l)||(l=p()[0]),t.innerHTML=`
    <aside class="sidebar">
      <div class="brand">
        <span class="brand-mark">H</span>
        <div>
  ...
  `,document.querySelectorAll("[data-view]").forEach(o=>{o.addEventListener("click",()=>{l=o.dataset.view,d()})}),document.querySelectorAll("[data-role]").forEach(o=>{o.addEventListener("click",()=>{h=o.dataset.role,p().includes(l)||(l=p()[0]),d()})}),(r=document.querySelector("[data-sign-out]"))==null||r.addEventListener("click",it),document.querySelectorAll("[data-view-link]").forEach(o=>{o.addEventListener("click",()=>{p().includes(o.dataset.viewLink)&&(l=o.dataset.viewLink),d()})});const a=document.querySelector("#payment-form");a&&a.addEventListener("submit",T);const s=document.querySelector("#vendor-form");s&&s.addEventListener("submit",V);const n=document.querySelector("#store-form");n&&n.addEventListener("submit",J),document.querySelectorAll("[data-payment-id][data-payment-status]").forEach(o=>{o.addEventListener("click",()=>{K(Number(o.dataset.paymentId),o.dataset.paymentStatus)})})}M();
