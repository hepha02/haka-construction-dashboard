import{createClient as A}from"https://esm.sh/@supabase/supabase-js@2.49.1";(function(){const t=document.createElement("link").relList;if(t&&t.supports&&t.supports("modulepreload"))return;for(const n of document.querySelectorAll('link[rel="modulepreload"]'))s(n);new MutationObserver(n=>{for(const r of n)if(r.type==="childList")for(const o of r.addedNodes)o.tagName==="LINK"&&o.rel==="modulepreload"&&s(o)}).observe(document,{childList:!0,subtree:!0});function a(n){const r={};return n.integrity&&(r.integrity=n.integrity),n.referrerPolicy&&(r.referrerPolicy=n.referrerPolicy),n.crossOrigin==="use-credentials"?r.credentials="include":n.crossOrigin==="anonymous"?r.credentials="omit":r.credentials="same-origin",r}function s(n){if(n.ep)return;n.ep=!0;const r=a(n);fetch(n.href,r)}})();const I="https://yqemtsbdnypgmkuyncxh.supabase.co",F="eyJhbGciOiJIUzI1NiIsInR5cCI6IkpXVCJ9.eyJpc3MiOiJzdXBhYmFzZSIsInJlZiI6InlxZW10c2JkbnlwZ21rdXluY3hoIiwicm9sZSI6ImFub24iLCJpYXQiOjE3ODAyNjYwMTUsImV4cCI6MjA5NTg0MjAxNX0.gwgdCncqRKKgC8ebj7qIdT-vA4J-wOVd2O9DSa7xEOs",c=A(I,F),u={payments:[{id:1,store:"성수 플래그십",vendor:"한빛전기",amount:184e5,status:"승인",requested_at:"2026-05-28"},{id:2,store:"부산 센텀",vendor:"도원인테리어",amount:327e5,status:"신청",requested_at:"2026-05-27"},{id:3,store:"대전 둔산",vendor:"서진설비",amount:98e5,status:"반려",requested_at:"2026-05-26"},{id:4,store:"제주 노형",vendor:"제이건축",amount:241e5,status:"승인",requested_at:"2026-05-25"}],stores:[{id:1,name:"성수 플래그십",area:52,status:"완료",budget:21e7,spent:1984e5},{id:2,name:"부산 센텀",area:47,status:"진행중",budget:186e6,spent:1227e5},{id:3,name:"대전 둔산",area:39,status:"진행중",budget:144e6,spent:882e5},{id:4,name:"제주 노형",area:42,status:"미착공",budget:158e6,spent:0}],vendors:[{id:1,name:"도원인테리어",category:"시공",bank:"신한은행",account_number:"110-000-000001",account_holder:"도원인테리어",risk:"정상",total:1245e5},{id:2,name:"한빛전기",category:"전기",bank:"국민은행",account_number:"004-000-000002",account_holder:"한빛전기",risk:"정상",total:738e5},{id:3,name:"서진설비",category:"설비",bank:"하나은행",account_number:"352-000-000003",account_holder:"서진설비",risk:"증빙확인",total:412e5}]},C=["대시보드","엑셀 업로드","결제 신청","업체/계좌 관리","매장별 공사 관리","진열장 원가 배분","견적서 생성","계약서 생성","은행 이체 파일 생성","관리자 설정"],R={"전체 관리자":C,"인테리어 공사실장":["결제 신청","진열장 원가 배분"]},O={admin:"전체 관리자",interior_manager:"인테리어 공사실장","전체 관리자":"전체 관리자","인테리어 공사실장":"인테리어 공사실장"},j={"엑셀 업로드":["결제 신청 내역 엑셀 업로드","필수 컬럼 검증","중복/오류 행 표시"],"진열장 원가 배분":["매장별 진열장 비용 배분","공용 비용 자동 분배","평당 원가 반영"],"견적서 생성":["매장/업체 기준 견적서 생성","공사항목별 금액 자동 합산","PDF/문서 다운로드"],"계약서 생성":["업체 정보 기반 계약서 생성","계좌/사업자 정보 자동 반영","계약 상태 관리"],"은행 이체 파일 생성":["승인된 결제 건만 추출","은행 업로드용 파일 생성","이체 전 검증"],"관리자 설정":["사용자 권한","승인 단계","상태/분류 코드 관리"]};let m=u,l="대시보드",v="인테리어 공사실장",g=null;const h=e=>new Intl.NumberFormat("ko-KR",{style:"currency",currency:"KRW",maximumFractionDigits:0}).format(e||0),M=()=>new Date().toISOString().slice(0,10),D=e=>Number(String(e).replace(/[^\d]/g,"")),b=e=>String(e||"").replace(/&/g,"&amp;").replace(/"/g,"&quot;").replace(/</g,"&lt;").replace(/>/g,"&gt;"),_=e=>({승인:"green",신청:"amber",반려:"red",진행중:"blue",완료:"green",미착공:"gray",정상:"green",증빙확인:"amber"})[e]||"gray";async function $(){if(!c)return u;const[e,t,a]=await Promise.all([c.from("payments").select("*").order("requested_at",{ascending:!1}).order("id",{ascending:!1}).limit(12),c.from("stores").select("*").order("id",{ascending:!0}),c.from("vendors").select("*").order("id",{ascending:!0})]);return{payments:e.error?u.payments:e.data,stores:t.error?u.stores:t.data,vendors:a.error?u.vendors:a.data}}async function N(e){if(!c||!(e!=null&&e.email))return"인테리어 공사실장";const{data:t,error:a}=await c.from("user_roles").select("role").ilike("email",e.email).maybeSingle();return a||!(t!=null&&t.role)?"인테리어 공사실장":O[t.role]||"인테리어 공사실장"}async function P(){var t;if(!c){m=await $(),v="전체 관리자",d();return}const{data:e}=await c.auth.getSession();if(g=((t=e.session)==null?void 0:t.user)||null,!g){f();return}v=await N(g),l=p()[0],m=await $(),d()}function T(e,t,a){return e.payments.find(s=>{const n=s.vendor.trim()===t.trim(),r=Math.abs(s.amount-a)/Math.max(a,1);return n&&r<=.1})}async function V(e){e.preventDefault();const t=e.currentTarget,a=t.querySelector("button[type='submit']"),s=t.querySelector("[data-form-message]"),n=new FormData(t),r=String(n.get("store")||"").trim(),o=String(n.get("vendor")||"").trim(),S=D(n.get("amount")),y=T(m,o,S);if(!r||!o||!S){s.textContent="매장명, 업체, 신청 금액을 모두 입력해 주세요.",s.className="form-message error";return}a.disabled=!0,a.textContent="저장 중",s.textContent=y?`중복 의심: ${y.store} / ${h(y.amount)} 건과 비슷합니다.`:"신청 건을 저장하고 있습니다.",s.className=y?"form-message warning":"form-message";const q={store:r,vendor:o,amount:S,status:"신청",requested_at:M()};if(!c)u.payments=[{id:Date.now(),...q},...u.payments];else{const{error:i}=await c.from("payments").insert(q);if(i){a.disabled=!1,a.textContent="검토 요청 생성",s.textContent=`저장 실패: ${i.message}`,s.className="form-message error";return}}t.reset(),m=await $(),d(y?"신청이 저장됐습니다. 중복 의심 건은 결제 검토에서 확인하세요.":"신청이 저장됐습니다.")}async function J(e){e.preventDefault();const t=e.currentTarget,a=t.querySelector("button[type='submit']"),s=t.querySelector("[data-vendor-message]"),n=new FormData(t),r={name:String(n.get("name")||"").trim(),category:String(n.get("category")||"").trim(),bank:String(n.get("bank")||"").trim(),account_number:String(n.get("account_number")||"").trim(),account_holder:String(n.get("account_holder")||"").trim(),risk:"정상",total:0};if(!r.name||!r.category||!r.bank||!r.account_number||!r.account_holder){s.textContent="업체명, 분류, 은행, 계좌번호, 예금주를 모두 입력해 주세요.",s.className="form-message error";return}if(a.disabled=!0,a.textContent="저장 중",s.textContent="업체/계좌 정보를 저장하고 있습니다.",s.className="form-message",!c)u.vendors=[{id:Date.now(),...r},...u.vendors];else{const{error:o}=await c.from("vendors").insert(r);if(o){a.disabled=!1,a.textContent="업체 저장",s.textContent=`저장 실패: ${o.message}`,s.className="form-message error";return}}t.reset(),m=await $(),l="업체/계좌 관리",d("업체/계좌 정보가 저장됐습니다.")}async function K(e){e.preventDefault();const t=e.currentTarget,a=t.querySelector("button[type='submit']"),s=t.querySelector("[data-store-message]"),n=new FormData(t),r={name:String(n.get("name")||"").trim(),area:Number(n.get("area")),status:String(n.get("status")||"미착공"),budget:D(n.get("budget")),spent:D(n.get("spent"))};if(!r.name||!r.area||!r.budget){s.textContent="매장명, 면적, 예산을 입력해 주세요.",s.className="form-message error";return}if(a.disabled=!0,a.textContent="저장 중",s.textContent="매장 공사 정보를 저장하고 있습니다.",s.className="form-message",!c)u.stores=[{id:Date.now(),...r},...u.stores];else{const{error:o}=await c.from("stores").insert(r);if(o){a.disabled=!1,a.textContent="매장 저장",s.textContent=`저장 실패: ${o.message}`,s.className="form-message error";return}}t.reset(),m=await $(),l="매장별 공사 관리",d("매장 공사 정보가 저장됐습니다.")}async function B(e,t){if(!(!e||!["승인","반려"].includes(t))){if(!c)u.payments=u.payments.map(a=>a.id===e?{...a,status:t}:a);else{const{error:a}=await c.from("payments").update({status:t}).eq("id",e).eq("status","신청");if(a){d(`상태 변경 실패: ${a.message}`);return}}m=await $(),l="결제 신청",d(`결제 신청이 ${t} 처리됐습니다.`)}}function H(e){const t=e.stores.filter(i=>i.status==="완료").length,a=e.stores.filter(i=>i.status==="진행중").length,s=e.stores.filter(i=>i.document_required).length,n=e.payments.filter(i=>i.status==="승인").reduce((i,k)=>i+k.amount,0),r=e.stores.reduce((i,k)=>i+Number(k.spent||0),0),o=e.payments.filter(i=>i.status==="신청").length,S=e.stores.filter(i=>String(i.name||"").includes("직영점")).length,y=e.stores.reduce((i,k)=>i+Number(k.area||0),0),q=Math.round(r/Math.max(y,1));return[["완료된 매장",`${t}개`,"엑셀 공사 상태 기준"],["진행중인 매장",`${a}개`,"시공 또는 비용 검수 중"],["전체 공사비",h(r),"엑셀 합계 기준"],["문서 생성 대상",`${s}개`,"강남압구정 행부터 아래"],["대기중인 결제",`${o}건`,"승인 전 검토 필요"],["직영점",`${S}개`,"지점명 기준"],["승인된 결제",h(n),"지급 승인 완료"],["평균 평당 원가",h(q),"엑셀 합계/평수 기준"]]}function w(e,t){return`
  ...
  `}function U(e){return e.payments.map(t=>`
      ...
      </tr>`)}function Y(e){return e.vendors.map(t=>`<option value="${b(t.name)}">${b(t.name)} / ${b(t.bank)} ${b(t.account_number||"")}</option>`).join("")}function Z(e){return e.stores.map(t=>`<option value="${b(t.name)}">${b(t.name)} / ${b(t.area)}평 / ${b(t.status)}</option>`).join("")}function E(){return`
  ...
  `}function G(e){return`
    ...
    <section class="grid two">
      ...
      <article class="panel">
        ...
        ${w(["지역","매장","진열장","평수","공사비 합계","상태","문서"],L(e))}
      </article>
    </section>
    ...
    <section class="grid lower">
      <article class="panel">
        ...
        ${w(["업체","분류","은행","계좌번호","예금주","누적 지급","상태"],x(e))}
      </article>
      ${E()}
    </section>
  `}function Q(e){return`
  ...
  `}function nt(e){return l==="대시보드"?G(e):l==="결제 신청"?Q(e):l==="업체/계좌 관리"?tt(e):l==="매장별 공사 관리"?et(e):at(l)}function p(){return R[v]||C}function st(){return`
