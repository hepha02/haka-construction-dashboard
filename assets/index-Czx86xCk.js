import{createClient as _}from"https://esm.sh/@supabase/supabase-js@2.49.1";(function(){const e=document.createElement("link").relList;if(e&&e.supports&&e.supports("modulepreload"))return;for(const s of document.querySelectorAll('link[rel="modulepreload"]'))r(s);new MutationObserver(s=>{for(const a of s)if(a.type==="childList")for(const i of a.addedNodes)i.tagName==="LINK"&&i.rel==="modulepreload"&&r(i)}).observe(document,{childList:!0,subtree:!0});function n(s){const a={};return s.integrity&&(a.integrity=s.integrity),s.referrerPolicy&&(a.referrerPolicy=s.referrerPolicy),s.crossOrigin==="use-credentials"?a.credentials="include":s.crossOrigin==="anonymous"?a.credentials="omit":a.credentials="same-origin",a}function r(s){if(s.ep)return;s.ep=!0;const a=n(s);fetch(s.href,a)}})();const q="https://yqemtsbdnypgmkuyncxh.supabase.co",C="eyJhbGciOiJIUzI1NiIsInR5cCI6IkpXVCJ9.eyJpc3MiOiJzdXBhYmFzZSIsInJlZiI6InlxZW10c2JkbnlwZ21rdXluY3hoIiwicm9sZSI6ImFub24iLCJpYXQiOjE3ODAyNjYwMTUsImV4cCI6MjA5NTg0MjAxNX0.gwgdCncqRKKgC8ebj7qIdT-vA4J-wOVd2O9DSa7xEOs",d=_(q,C),c={payments:[{id:1,store:"성수 플래그십",vendor:"한빛전기",amount:184e5,status:"승인",requested_at:"2026-05-28"},{id:2,store:"부산 센텀",vendor:"도원인테리어",amount:327e5,status:"신청",requested_at:"2026-05-27"},{id:3,store:"대전 둔산",vendor:"서진설비",amount:98e5,status:"반려",requested_at:"2026-05-26"},{id:4,store:"제주 노형",vendor:"제이건축",amount:241e5,status:"승인",requested_at:"2026-05-25"}],stores:[{id:1,name:"성수 플래그십",area:52,status:"완료",budget:21e7,spent:1984e5},{id:2,name:"부산 센텀",area:47,status:"진행중",budget:186e6,spent:1227e5},{id:3,name:"대전 둔산",area:39,status:"진행중",budget:144e6,spent:882e5},{id:4,name:"제주 노형",area:42,status:"미착공",budget:158e6,spent:0}],vendors:[{id:1,name:"도원인테리어",category:"시공",bank:"신한은행",account_number:"110-000-000001",account_holder:"도원인테리어",risk:"정상",total:1245e5},{id:2,name:"한빛전기",category:"전기",bank:"국민은행",account_number:"004-000-000002",account_holder:"한빛전기",risk:"정상",total:738e5},{id:3,name:"서진설비",category:"설비",bank:"하나은행",account_number:"352-000-000003",account_holder:"서진설비",risk:"증빙확인",total:412e5}]},x=["대시보드","엑셀 업로드","결제 신청","업체/계좌 관리","매장별 공사 관리","진열장 원가 배분","견적서 생성","계약서 생성","은행 이체 파일 생성","관리자 설정"],D={"엑셀 업로드":["결제 신청 내역 엑셀 업로드","필수 컬럼 검증","중복/오류 행 표시"],"진열장 원가 배분":["매장별 진열장 비용 배분","공용 비용 자동 분배","평당 원가 반영"],"견적서 생성":["매장/업체 기준 견적서 생성","공사항목별 금액 자동 합산","PDF/문서 다운로드"],"계약서 생성":["업체 정보 기반 계약서 생성","계좌/사업자 정보 자동 반영","계약 상태 관리"],"은행 이체 파일 생성":["승인된 결제 건만 추출","은행 업로드용 파일 생성","이체 전 검증"],"관리자 설정":["사용자 권한","승인 단계","상태/분류 코드 관리"]};let m=c,l="대시보드";const u=t=>new Intl.NumberFormat("ko-KR",{style:"currency",currency:"KRW",maximumFractionDigits:0}).format(t||0),N=()=>new Date().toISOString().slice(0,10),$=t=>Number(String(t).replace(/[^\d]/g,"")),y=t=>({승인:"green",신청:"amber",반려:"red",진행중:"blue",완료:"green",미착공:"gray",정상:"green",증빙확인:"amber"})[t]||"gray";async function h(){if(!d)return c;const[t,e,n]=await Promise.all([d.from("payments").select("*").order("requested_at",{ascending:!1}).order("id",{ascending:!1}).limit(12),d.from("stores").select("*").order("id",{ascending:!0}),d.from("vendors").select("*").order("id",{ascending:!0})]);return{payments:t.error?c.payments:t.data,stores:e.error?c.stores:e.data,vendors:n.error?c.vendors:n.data}}function E(t,e,n){return t.payments.find(r=>{const s=r.vendor.trim()===e.trim(),a=Math.abs(r.amount-n)/Math.max(n,1);return s&&a<=.1})}async function I(t){t.preventDefault();const e=t.currentTarget,n=e.querySelector("button[type='submit']"),r=e.querySelector("[data-form-message]"),s=new FormData(e),a=String(s.get("store")||"").trim(),i=String(s.get("vendor")||"").trim(),v=$(s.get("amount")),b=E(m,i,v);if(!a||!i||!v){r.textContent="매장명, 업체, 신청 금액을 모두 입력해 주세요.",r.className="form-message error";return}n.disabled=!0,n.textContent="저장 중",r.textContent=b?`중복 의심: ${b.store} / ${u(b.amount)} 건과 비슷합니다.`:"신청 건을 저장하고 있습니다.",r.className=b?"form-message warning":"form-message";const o={store:a,vendor:i,amount:v,status:"신청",requested_at:N()};if(!d)c.payments=[{id:Date.now(),...o},...c.payments];else{const{error:p}=await d.from("payments").insert(o);if(p){n.disabled=!1,n.textContent="검토 요청 생성",r.textContent=`저장 실패: ${p.message}`,r.className="form-message error";return}}e.reset(),m=await h(),f(b?"신청이 저장됐습니다. 중복 의심 건은 결제 검토에서 확인하세요.":"신청이 저장됐습니다.")}async function A(t){t.preventDefault();const e=t.currentTarget,n=e.querySelector("button[type='submit']"),r=e.querySelector("[data-vendor-message]"),s=new FormData(e),a={name:String(s.get("name")||"").trim(),category:String(s.get("category")||"").trim(),bank:String(s.get("bank")||"").trim(),account_number:String(s.get("account_number")||"").trim(),account_holder:String(s.get("account_holder")||"").trim(),risk:"정상",total:0};if(!a.name||!a.category||!a.bank||!a.account_number||!a.account_holder){r.textContent="업체명, 분류, 은행, 계좌번호, 예금주를 모두 입력해 주세요.",r.className="form-message error";return}if(n.disabled=!0,n.textContent="저장 중",r.textContent="업체/계좌 정보를 저장하고 있습니다.",r.className="form-message",!d)c.vendors=[{id:Date.now(),...a},...c.vendors];else{const{error:i}=await d.from("vendors").insert(a);if(i){n.disabled=!1,n.textContent="업체 저장",r.textContent=`저장 실패: ${i.message}`,r.className="form-message error";return}}e.reset(),m=await h(),l="업체/계좌 관리",f("업체/계좌 정보가 저장됐습니다.")}async function O(t){t.preventDefault();const e=t.currentTarget,n=e.querySelector("button[type='submit']"),r=e.querySelector("[data-store-message]"),s=new FormData(e),a={name:String(s.get("name")||"").trim(),area:Number(s.get("area")),status:String(s.get("status")||"미착공"),budget:$(s.get("budget")),spent:$(s.get("spent"))};if(!a.name||!a.area||!a.budget){r.textContent="매장명, 면적, 예산을 입력해 주세요.",r.className="form-message error";return}if(n.disabled=!0,n.textContent="저장 중",r.textContent="매장 공사 정보를 저장하고 있습니다.",r.className="form-message",!d)c.stores=[{id:Date.now(),...a},...c.stores];else{const{error:i}=await d.from("stores").insert(a);if(i){n.disabled=!1,n.textContent="매장 저장",r.textContent=`저장 실패: ${i.message}`,r.className="form-message error";return}}e.reset(),m=await h(),l="매장별 공사 관리",f("매장 공사 정보가 저장됐습니다.")}async function F(t,e){if(!(!t||!["승인","반려"].includes(e))){if(!d)c.payments=c.payments.map(n=>n.id===t?{...n,status:e}:n);else{const{error:n}=await d.from("payments").update({status:e}).eq("id",t).eq("status","신청");if(n){f(`상태 변경 실패: ${n.message}`);return}}m=await h(),l="결제 신청",f(`결제 신청이 ${e} 처리됐습니다.`)}}function L(t){const e=t.stores.filter(o=>o.status==="완료").length,n=t.stores.filter(o=>o.status==="진행중").length,r=t.payments.filter(o=>o.status==="승인").reduce((o,p)=>o+p.amount,0),s=t.stores.reduce((o,p)=>o+p.spent,0),a=t.payments.filter(o=>o.status==="신청").length,i=t.vendors.filter(o=>o.risk==="증빙확인").length,v=t.payments.filter(o=>o.amount>3e7).length,b=Math.round(s/Math.max(t.stores.reduce((o,p)=>o+p.area,0),1));return[["완료된 매장",`${e}개`,"준공 및 정산 완료"],["진행중인 매장",`${n}개`,"시공 또는 비용 검수 중"],["승인된 공사비",u(r),"지급 승인 완료"],["실제 사용액",u(s),"전체 매장 누적"],["대기중인 결제",`${a}건`,"승인 전 검토 필요"],["증빙 미비",`${i}건`,"자료 보완 요청"],["중복 의심",`${v}건`,"금액/업체 패턴 확인"],["평균 평당 원가",u(b),"실사용액 기준"]]}function g(t,e){return`
  ...
  `}function R(t){return t.payments.map(e=>`
      <tr>
        <td>${e.store}</td>
        <td>${e.vendor}</td>
        <td class="money">${u(e.amount)}</td>
        <td><span class="badge ${y(e.status)}">${e.status}</span></td>
        <td>${e.requested_at}</td>
      </tr>`)}function j(t){return t.payments.map(e=>`
      <tr>
        <td>${e.store}</td>
        <td>${e.vendor}</td>
        <td class="money">${u(e.amount)}</td>
      ...
      </tr>`)}function w(t){return t.stores.map(e=>`
      <tr>
        <td>${e.name}</td>
        <td>${e.area}평</td>
        <td class="money">${u(e.budget)}</td>
        <td class="money">${u(e.spent)}</td>
        <td><span class="badge ${y(e.status)}">${e.status}</span></td>
      </tr>`)}function S(t){return t.vendors.map(e=>`
      <tr>
        <td>${e.name}</td>
        <td>${e.category}</td>
        <td>${e.bank}</td>
        ...
        <td class="money">${u(e.total)}</td>
        <td><span class="badge ${y(e.risk)}">${e.risk}</span></td>
      </tr>`)}function M(t){return t.vendors.map(e=>`<option value="${e.name}">${e.name} / ${e.bank} ${e.account_number||""}</option>`).join("")}function P(t){return t.stores.map(e=>`<option value="${e.name}">${e.name} / ${e.area}평 / ${e.status}</option>`).join("")}function k(){return`
  ...
  `}function K(t){return`
    ...
    <section class="grid lower">
      <article class="panel">
        ...
        ${g(["업체","분류","은행","계좌번호","예금주","누적 지급","상태"],S(t))}
      </article>
      ${k()}
    </section>
  `}function T(t){return`
  ...
  `}function X(t){const e=D[t]||["기능 범위 정의","입력 항목 확정","데이터 연결"];return`
  ...
  `}function Y(t){return l==="대시보드"?K(t):l==="결제 신청"?T(t):l==="업체/계좌 관리"?B(t):l==="매장별 공사 관리"?U(t):X(l)}function f(t=""){const e=document.querySelector("#app");e.innerHTML=`
    <aside class="sidebar">
      <div class="brand">
  ...
  `,document.querySelectorAll("[data-view]").forEach(a=>{a.addEventListener("click",()=>{l=a.dataset.view,f()})}),document.querySelectorAll("[data-view-link]").forEach(a=>{a.addEventListener("click",()=>{l=a.dataset.viewLink,f()})});const n=document.querySelector("#payment-form");n&&n.addEventListener("submit",I);const r=document.querySelector("#vendor-form");r&&r.addEventListener("submit",A);const s=document.querySelector("#store-form");s&&s.addEventListener("submit",O),document.querySelectorAll("[data-payment-id][data-payment-status]").forEach(a=>{a.addEventListener("click",()=>{F(Number(a.dataset.paymentId),a.dataset.paymentStatus)})})}h().then(t=>{m=t,f()});
