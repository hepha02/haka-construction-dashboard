(() => {
  ...
  function detailRows(card) {
    const rows = [];
    for (const box of card.querySelectorAll(".payment-detail-grid > div")) {
      rows.push({
  ...
  function downloadExcel(records) {
    ...
    const rows = records.map((record) => `<tr>${td(record.bank)}${td(record.account, "mso-number-format:'\\@';")}${td(record.amount, "mso-number-format:'0';")}${td(record.holder)}</tr>`).join("");
    const html = `<!doctype html><html xmlns:o="urn:schemas-microsoft-com:office:office" xmlns:x="urn:schemas-microsoft-com:office:excel"><head><meta http-equiv="Content-Type" content="application/vnd.ms-excel; charset=UTF-8"><meta charset="UTF-8"><style>td{font-family:'Malgun Gothic',Arial,sans-serif;font-size:11pt;}</style></head><body><table><tr>${headers.map((header) => td(header)).join("")}</tr>${rows}</table></body></html>`;
